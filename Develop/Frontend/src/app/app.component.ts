...
export class AppComponent {
  ...
  private setTimeout(idle: Idle, keepalive: Keepalive) {
    ...
    idle.onTimeoutWarning.subscribe((countdown: any) => {
      this.progressBarPopup.componentInstance.count =
        Math.floor((countdown - 1) / 60) + 1;
      this.progressBarPopup.componentInstance.progressCount =
        this.reverseNumber(countdown);
      this.progressBarPopup.componentInstance.progressCountMax =
        this.timeoutCount;
      this.progressBarPopup.componentInstance.countMinutes = Math.floor(
        countdown / 60,
      );
